...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter, resultFilter, loading, totalElements } = useSelector(cutOffPaymentRefundToEmployerSelector);
          ...
          <>
            <div className='flex justify-center gap-2'>
              <DollarCircle
                ...
                onClick={() => {
                  if (row.payType === 'X') {
                    router.push(`${URL.cutOffPayment.cutOffPaymentRefundToEmployerOffice.url}?id=${row.documentNo}`);
                  } else if (row.payType === 'T') {
                    router.push(`${URL.cutOffPayment.cutOffPaymentRefundToEmployerBank.url}?id=${row.documentNo}`);
                  } else if (row.payType === 'S') {
                    router.push(`${URL.cutOffPayment.cutOffPaymentRefundToEmployerCheque.url}?id=${row.documentNo}`);
                  } else if (row.payType === 'P') {
                    router.push(`${URL.cutOffPayment.cutOffPaymentRefundToEmployerMoney.url}?id=${row.documentNo}`);
                  } else {
