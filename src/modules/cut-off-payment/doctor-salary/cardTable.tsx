...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter, resultFilter, loading, totalElements } = useSelector(cutOffPaymentDoctorSalarySelector);
          ...
          <>
            <div className='flex justify-center gap-2'>
              <DollarCircle
                ...
                onClick={() => {
                  if (row.payType === 'X') {
                    router.push(`${URL.cutOffPayment.cutOffPaymentDoctorSalaryOffice.url}?id=${row.documentNo}`);
                  } else if (row.payType === 'T') {
                    router.push(`${URL.cutOffPayment.cutOffPaymentDoctorSalaryBank.url}?id=${row.documentNo}`);
                  } else if (row.payType === 'S') {
                    router.push(`${URL.cutOffPayment.cutOffPaymentDoctorSalaryCheque.url}?id=${row.documentNo}`);
                  } else if (row.payType === 'P') {
                    router.push(`${URL.cutOffPayment.cutOffPaymentDoctorSalaryMoney.url}?id=${row.documentNo}`);
                  } else {
