...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter, resultFilter, loading, totalElements } = useSelector(cutOffPaymentHospitalSelector);
              ...
              <DollarCircle
                ...
                onClick={() => {
                  if (row.payType === 'X') {
                    ...
                    router.push(`${URL.cutOffPayment.cutOffHospitalPaymentBank.url}?id=${row.documentNo}`);
