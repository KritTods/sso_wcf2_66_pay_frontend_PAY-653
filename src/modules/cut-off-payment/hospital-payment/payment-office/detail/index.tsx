...
export default function PaymentOfficeDetail(): React.ReactElement {
  const dataTestId = 'pageCutoffPaymentHospitalPaymentDetail';
  ...
  useEffect(() => {
    if (!id) return;
    // Call API Thunks
    void dispatch(getHospitalDetailService(id));

    const fetchData = async (): Promise<void> => {
      const { payload } = (await dispatch(getHospitalDetailService(id))) as PayloadAction<PageFormType>;
                ...
                summary={() => {
                  ...
                  return (
                    <Table.Summary.Row className='bg-gray-200'>
                      ...
                      <Table.Summary.Cell index={2} colSpan={2} className='rounded-br-xl'>
                        <p className='text-lg font-bold text-right'></p>
                      </Table.Summary.Cell>
                    </Table.Summary.Row>
                  );
                }}
