...
export default function OfficeDetail(): React.JSX.Element {
  ...
  const [isHistoryButtonHidden, setIsHistoryButtonHidden] = useState(false); // ใช้สถานะเพื่อซ่อนปุ่มประวัติการแก้ไขเช็ค
  ...
  useEffect(() => {
    ...
    const fetchData = async (): Promise<void> => {
      ...
      const resDetail = (await dispatch(getReadyToPayByIdService(id))) as PayloadAction<GetReadyToPayByIdServiceType>;
  ...
  const onFinish: FormProps<FormType>['onFinish'] = (values) => {
    ...
    setIsHistoryButtonHidden(false); // ให้แสดงปุ่ม "ประวัติการแก้ไขเช็ค" หลังบันทึกเสร็จ
  };
            ...
            {mode === 'view' && (
              <>
                ...
                <BaseButton
                  ...
                  onClick={() => {
                    setMode('edit');
                    setIsHistoryButtonHidden(true); // ซ่อนปุ่มประวัติการแก้ไขเช็คเมื่อคลิกปุ่มแก้ไข
                  }}
                ...
                className='w-[240px]'
                onClick={() => {
                  router.push(`${URL.taxDeliver.taxDeliverReadyToPayOffice.url}?id=${id}`);
                }}
              /> */}
