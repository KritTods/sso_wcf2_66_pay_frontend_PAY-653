...
export default function RefundToEmployerDetail(): React.ReactElement {
  const dataTestId = 'pageCancelCutoffPaymentRefundToEmployerDetail';
  const router = useRouter();
  const searchParams = useSearchParams();
  const id = searchParams.get('id');
  const dispatch = useAppDispatch();
  const { loading, pageDetail } = useSelector(cancelCutOffPayRefundToEmployerSelector);
  ...
  useEffect(() => {
    ...
    const fetchData = async (): Promise<void> => {
      const { payload } = (await dispatch(getRefundToEmployerDetailService(id))) as PayloadAction<PageDetailType>;
      ...
      render: (_: unknown, record: RefundToEmploerDataType): React.ReactElement => {
        return (
          <span className='w-full flex justify-center'>
            {record.accountBranchCode} : {record.accountName}
          </span>
        );
      },
    },
      ...
      <div className='flex flex-col gap-4 mx-4 mb-6'>
        ...
        <div className='flex justify-center gap-4'>
          <BaseButton
            ...
            label='ยกเลิก'
            className='w-[240px]'
            onClick={() => router.push(URL.cancelCutOffPayment.cancelCutOffRefundToEmployer.url)}
          />
        </div>
      </div>
    </>
