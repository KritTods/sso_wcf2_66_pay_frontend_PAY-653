...
export default function CardSearchTab({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter } = useSelector(cancelCutOffPayRefundToEmployerSelector);
  ...
  const onFinish: FormProps<FilterSearchType>['onFinish'] = (values) => {
    const searchObj = {
      ...
      accountBranchCode: values.accountBranchCode,
  ...
  return (
    <div className='flex p-6 bg-white shadow-sm rounded-xl'>
      <BaseForm name='cancelCutOffPayRefundToEmployerFilter' onFinish={onFinish} initialValues={filter}>
          ...
          <div className='w-full'>
            <div className='flex flex-col gap-4'>
              <Row gutter={[16, 16]}>
                ...
                <Col {...searchColumn}>
                  <BaseItemInput
                    id={`${dataTestId}-accountNo-input-text`}
                ...
                <Col {...searchColumn}>
                  <BaseItemInput
                    id={`${dataTestId}-accountBranchCode-input-text`}
                    label='ลำดับที่สาขา'
                    itemName='accountBranchCode'
                ...
                <Col {...searchColumn}>
                  <BaseItemInput
                    id={`${dataTestId}-accountName-input-text`}
