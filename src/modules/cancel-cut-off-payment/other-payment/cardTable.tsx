...
import {
  ...
  getcancelCutOffPayOtherPaymentService,
  setFilter,
  cancelCutOffPayOtherPaymentDelectService,
...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter, tableList, loading, totalElements } = useSelector(cancelCutOffPayOtherPaymentSelector);
  ...
  useEffect(() => {
    if (loading) {
      void dispatch(getcancelCutOffPayOtherPaymentService({ data: filter }));
    }
  ...
  const handleConfirmDelete = (): void => {
    if (selectedId) {
      ...
      dispatch(cancelCutOffPayOtherPaymentDelectService(selectedId))
        .unwrap()
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'center',
      width: 150,
      render: (_: unknown, record: OtherPaymentDataType): React.ReactElement => {
    ...
    },
    {
      ...
      render: (payType: { code: string; name: string } | undefined): React.ReactElement => {
        return <span className='w-full flex justify-center'>{statusPayType[payType?.code as PayType]}</span>;
      },
    },
          ...
          <>
            <div className='flex justify-center gap-2'>
              <Page
                ...
                onClick={() => {
                  router.push(`${URL.cancelCutOffPayment.cancelCutOffOtherPaymentDetail.url}?id=${row.documentNo}`);
                }}
              />
