...
import {
  ...
  getcancelCutOffPayOtherPaymentDetailService,
  PageDetailType,
  setPageDetail,
...
export default function DoctorSalaryDetail(): React.ReactElement {
  const dataTestId = 'pageCancelCutoffPaymentOtherDetail';
  const router = useRouter();
  const searchParams = useSearchParams();
  const id = searchParams.get('id');
  const dispatch = useAppDispatch();
  const { loading, pageDetail } = useSelector(cancelCutOffPayOtherPaymentSelector);
  ...
  useEffect(() => {
    ...
    const fetchData = async (): Promise<void> => {
      const { payload } = (await dispatch(
        getcancelCutOffPayOtherPaymentDetailService(id),
      ...
      <div className='flex flex-col gap-4 mx-4 mb-6'>
        ...
        <div className='flex justify-center gap-4'>
          <BaseButton
            ...
            label='ยกเลิก'
            className='w-[240px]'
            onClick={() => router.push(URL.cancelCutOffPayment.cancelCutOffOtherPayment.url)}
          />
        </div>
      </div>
    </>
