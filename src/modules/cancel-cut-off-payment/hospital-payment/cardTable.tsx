...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter, tableList, loading, totalElements } = useSelector(cancelCutOffPayHospitalSelector);
  ...
  useEffect(() => {
    if (loading) {
      void dispatch(getHospitalService({ data: filter }));
    }
          ...
          <>
            <div className='flex justify-center gap-2'>
              <Page
                ...
                onClick={() => {
                  router.push(`${URL.cancelCutOffPayment.cancelCutOffHospitalDetail.url}?id=${row.documentNo}`);
                }}
              />
              <button
