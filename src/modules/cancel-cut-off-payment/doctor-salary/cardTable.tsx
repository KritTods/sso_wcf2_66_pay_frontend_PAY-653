...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter, tableList, loading, totalElements } = useSelector(cancelCutOffPayDoctorSalarySelector);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (
        text: string,
        record: { payType: { code: string; name: string }; documentNo: string },
        index: number,
      ): React.ReactElement => {
        const row = record;
          ...
          <>
            <div className='flex justify-center gap-2'>
              <Page
                ...
                onClick={() => {
                  router.push(
                    `${URL.cancelCutOffPayment.cancelCutOffPaymentDoctorSalaryDetail.url}?id=${row.documentNo}`,
                  );
                }}
              />
