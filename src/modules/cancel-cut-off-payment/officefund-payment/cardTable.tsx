...
import {
  ...
  OfficeFundDataType,
  getcancelCutOffPayOfficeFundService,
  setFilter,
  cancelCutOffPayOfficeFundDelectService,
...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter, tableList, loading, totalElements } = useSelector(cancelCutOffPayOfficeFundSelector);
  ...
  useEffect(() => {
    if (loading) {
      void dispatch(getcancelCutOffPayOfficeFundService({ data: filter }));
    }
  ...
  const handleConfirmDelete = (): void => {
    if (selectedId) {
      ...
      dispatch(cancelCutOffPayOfficeFundDelectService(selectedId))
        .unwrap()
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'center',
      width: 150,
      render: (_: unknown, record: OfficeFundDataType): React.ReactElement => {
          ...
          <>
            <div className='flex justify-center gap-2'>
              <Page
                ...
                onClick={() => {
                  router.push(
                    `${URL.cancelCutOffPayment.cancelCutOffOfficeFundPaymentDetail.url}?id=${row.documentNo}`,
                  );
                }}
              />
