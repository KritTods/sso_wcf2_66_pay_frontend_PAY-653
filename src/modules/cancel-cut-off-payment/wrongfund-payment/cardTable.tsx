...
import {
  ...
  WrongFundDataType,
  getcancelCutOffPayWrongFundService,
  setFilter,
  cancelCutOffPayWrongFundDelectService,
...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter, tableList, loading, totalElements } = useSelector(cancelCutOffPayWrongFundSelector);
  ...
  useEffect(() => {
    if (loading) {
      void dispatch(getcancelCutOffPayWrongFundService({ data: filter }));
    }
  ...
  const handleConfirmDelete = (): void => {
    if (selectedId) {
      ...
      dispatch(cancelCutOffPayWrongFundDelectService(selectedId))
        .unwrap()
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'center',
      width: 150,
      render: (_: unknown, record: WrongFundDataType): React.ReactElement => {
          ...
          <>
            <div className='flex justify-center gap-2'>
              <Page
                ...
                onClick={() => {
                  router.push(`${URL.cancelCutOffPayment.cancelCutOffWrongfundPaymentDetail.url}?id=${row.documentNo}`);
                }}
              />
