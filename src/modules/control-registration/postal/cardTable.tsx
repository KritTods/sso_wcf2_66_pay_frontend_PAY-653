...
import {
  ...
  setFilter,
  getControlRegistrationPostalService,
...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter, tableList, loading, totalElements } = useSelector(controlRegistrationPostalSelector);
  const { pageSize, pageNumber, orders } = filter.pagination;

  useEffect(() => {
    if (loading) {
      void dispatch(getControlRegistrationPostalService({ data: filter }));
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'center',
      widht: 100,
      render: (text: string, record: { payType: string; postalNo: string }): React.ReactElement => {
        const row = record;
          ...
          <>
            <div className='flex justify-center gap-2'>
              <Page
                ...
                onClick={() => {
                  router.push(`${URL.controlRegistration.controlRegistrationPostalDetail.url}?id=${row.postalNo}`);
                }}
              />
            </div>
          </>
