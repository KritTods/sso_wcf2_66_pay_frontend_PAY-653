...
import { FilterSearchType, setFilter } from '@/store-redux/slices/control-registration/cheque';
...
export default function CardSearchTab({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const onFinish: FormProps<FilterSearchType>['onFinish'] = (values) => {
    ...
    const searchObj = {
      ...
      chequeStampDate: [
        dayjs(values.chequeStampDate[0]).format('YYYY-MM-DD'),
        dayjs(values.chequeStampDate[1]).format('YYYY-MM-DD'),
      ],
          ...
          <div className='w-full'>
            <div className='flex flex-col gap-4'>
              <Row gutter={[16, 16]}>
                ...
                <Col {...searchColumn}>
                  <BaseItemDateRangePicker
                    id={`${dataTestId}-filter-chequeStampDate-input-date`}
                    itemName='chequeStampDate'
                    label='วันที่เช็ค'
                ...
                <Col {...searchColumn}>
                  <BaseItemDropdown
                    ...
                    option={[
                      ...
                      { value: 'T13', label: 'จ่ายโรงพยาบาล' },
                      { value: 'S1', label: 'จ่ายคืนเงินสมทบให้นายจ้าง' },
                      { value: 'E1', label: 'จ่ายเงินประเภทอื่น' },
                      { value: 'B2', label: 'จ่ายเงินกองทุนเพื่อบริหารสำนักงาน' },
