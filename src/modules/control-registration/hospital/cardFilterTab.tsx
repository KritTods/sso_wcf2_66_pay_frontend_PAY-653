...
export default function CardSearchTab({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter } = useSelector(controlRegistrationHospitalSelector);
          ...
          <div className='w-full'>
            <div className='flex flex-col gap-4'>
              <Row gutter={[16, 16]}>
                ...
                <Col {...searchColumn}>
                  <BaseItemDateRangePicker
                    id={`${dataTestId}-filter-cutOffDate-input-date`}
                    itemName='cutOffDate'
                    label='วันที่ตัดจ่าย'
                ...
                <Col span={12}>
                  <BaseItemInput
                    id={`${dataTestId}-filter-hospitalName-input-text`}
                    label='ชื่อโรงพยาบาล'
                    itemName='hospitalName'
                    placeholder='ชื่อโรงพยาบาล หรือ รหัสโรงพยาบาล'
                  />
                </Col>
                <Col {...searchColumn}>
                  <BaseItemDropdown
