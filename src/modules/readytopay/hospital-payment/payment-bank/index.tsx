...
const PaymentBank: React.FC = () => {
  ...
  const handleConfirm = async (): Promise<void> => {
    ...
    if (resCreateApi.payload) {
      ...
      router.push(`${URL.readytopay.hospitalPaymentBankDetail.url}?id=${resCreateApi.payload.prepareToPayId}`);
  ...
  const dataCardTax = useMemo(() => {
    ...
    //get bank code from dataPaymentOrders for set bankCode in cheques
    const bankCode = dataPaymentOrders[0]?.bank?.code;
    //update state ของ form when data change
    setInitFormData((prev) => {
      const newData = { ...prev };

      const initDataQheque: TableChequeType[] = INIT_DATA_CHEQUE.map((item) => {
        return {
          ...item,
          bankCode: bankCode,
          mode: 'addBank',
      ...
      const cheques: TableChequeType[] =
        (Array.isArray(form.getFieldValue('cheques')) &&
          (form.getFieldValue('cheques') as TableChequeType[]).map((item: TableChequeType) => {
            return {
              ...item,
              bankCode: bankCode,
              mode: 'addBank',
