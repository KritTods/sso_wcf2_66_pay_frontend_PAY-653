...
const PaymentOffice: React.FC = () => {
    ...
    moneys: [
      {
        ...
        receiverName: 'อรุณเซอร์วิส แอนด์ ซัพพลาย',
        amount: 120000,
        mode: 'custom',
        postalNo: '',
      },
      {
        id: '2',
        postalCode: '00000',
        portalDestination: 'ปทุมธานี',
        receiverName: 'เรด ไดมอนด์ คอร์ปอเรชั่น',
        amount: 1500000,
        mode: 'custom',
        postalNo: '',
      },
  ...
  useEffect(() => {
    ...
    // const manipulateMoneys = INIT_DATA_MONEY.map((item) => {
    //   return {
    //     ...item,
    //     mode: 'custom',
    //   };
    ...
    // setInitFormData(manipulateMoneys);
  ...
  const handleConfirm = async (): Promise<void> => {
    ...
    if (resCreateApi.payload) {
      ...
      router.push(`${URL.readytopay.refundToEmployerMoneyDetail.url}?id=${resCreateApi.payload.prepareToPayId}`);
