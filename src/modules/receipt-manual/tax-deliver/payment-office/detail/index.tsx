...
export default function PaymentOfficeDetail(): React.ReactElement {
  ...
  useEffect(() => {
    if (!id || !user) return;
    // Call API Thunks
    void dispatch(getReceiptManualTaxDeliverDetailService(id));
  ...
  return (
    ...
    <BaseForm name='paymentBankForm' initialValues={initialValuesForm} extraForm={form} onFinish={onSubmit}>
      <div className='flex flex-col gap-4 mx-4 mb-6'>
        ...
        {!pageForm.isCheque && 
        <CardCash dataTestId={dataTestId} cash={pageForm.cash} />
        }
        ...
        <div className='flex justify-center gap-4'>
          ...
          {editCheque ? (
            ...
            <>
              <BaseButton size='large' label='แก้ไข' type='outline' onClick={() => setEditCheque(true)} />
            </>
          )}
          {pageForm.isCheque && (   
