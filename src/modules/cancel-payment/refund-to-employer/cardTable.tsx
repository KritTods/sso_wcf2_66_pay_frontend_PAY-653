...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter, tableList, loading } = useSelector(cancelRefundToEmployerSelector);
          ...
          <>
            <div className='flex justify-center gap-2'>
              <Page
                ...
                onClick={() => {
                  if (row.payType === 'X') {
                    router.push(`${URL.cancelPayment.cancelRefundToEmployerOfficeDetail.url}?id=${row.paymentNo}`);
                  } else if (row.payType === 'T') {
                    router.push(`${URL.cancelPayment.cancelRefundToEmployerBankeDetail.url}?id=${row.paymentNo}`);
                  } else if (row.payType === 'S') {
                    router.push(`${URL.cancelPayment.cancelRefundToEmployerChequeDetail.url}?id=${row.paymentNo}`);
                  } else if (row.payType === 'P') {
                    router.push(`${URL.cancelPayment.cancelRefundToEmployerMoneyDetail.url}?id=${row.paymentNo}`);
                  } else {
