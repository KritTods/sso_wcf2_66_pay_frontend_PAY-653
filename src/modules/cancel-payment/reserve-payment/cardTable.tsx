...
import {
  ...
  getReservePaymentService,
  ReservePaymentDataType,
...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter, tableList, loading } = useSelector(cancelReservePaymentSelector);
  const [isOpenConfirmDeleteModal, setIsOpenConfirmDeleteModal] = useState(false);

  useEffect(() => {
    if (loading) {
      void dispatch(getReservePaymentService());
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string, record: unknown): React.ReactElement => {
        const row = record as ReservePaymentDataType;
          ...
          <>
            <div className='flex justify-center gap-2'>
              <Page
                ...
                onClick={() => {
                  router.push(`${URL.cancelPayment.cancelReservePaymentDetail.url}?id=${row.documentNo}`);
                }}
              />
