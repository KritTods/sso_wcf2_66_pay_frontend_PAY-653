...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { loading, filter, tableList } = useSelector(cancelOtherPaymentSelector);
  const [isOpenConfirmDeleteModal, setIsOpenConfirmDeleteModal] = useState(false);

  useEffect(() => {
    if (loading) {
      void dispatch(getOtherPaymentService({ data: filter }));
    }
        ...
        return (
          <div className='flex justify-center gap-2'>
            <Page
              ...
              onClick={() => {
                if (row.payType === 'X') {
                  router.push(`${URL.cancelPayment.cancelOtherPaymentOfficeDetail.url}?id=${row.paymentNo}`);
                } else if (row.payType === 'T') {
                  router.push(`${URL.cancelPayment.cancelOtherPaymentBankeDetail.url}?id=${row.paymentNo}`);
                } else if (row.payType === 'S') {
                  router.push(`${URL.cancelPayment.cancelOtherPaymentChequeDetail.url}?id=${row.paymentNo}`);
                } else if (row.payType === 'P') {
                  router.push(`${URL.cancelPayment.cancelOtherPaymentMoneyDetail.url}?id=${row.paymentNo}`);
                } else {
