...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { loading, filter, tableList } = useSelector(cancelHospitalPaymentSelector);
  const [isOpenConfirmDeleteModal, setIsOpenConfirmDeleteModal] = useState(false);

  useEffect(() => {
    if (loading) {
      void dispatch(getHospitalPaymentService({ data: filter }));
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string, record: unknown): React.ReactElement => {
        const row = record as HospitalPaymentDataType;
          ...
          <>
            <div className='flex justify-center gap-2'>
              <Page
                ...
                onClick={() => {
                  if (row.payType === 'X') {
                    router.push(`${URL.cancelPayment.cancelHospitalPaymentOfficeDetail.url}?id=${row.paymentNo}`);
                  } else if (row.payType === 'T') {
                    router.push(`${URL.cancelPayment.cancelHospitalPaymentBankeDetail.url}?id=${row.paymentNo}`);
                  } else if (row.payType === 'S') {
                    router.push(`${URL.cancelPayment.cancelHospitalPaymentChequeDetail.url}?id=${row.paymentNo}`);
                  } else if (row.payType === 'P') {
                    router.push(`${URL.cancelPayment.cancelHospitalPaymentMoneyDetail.url}?id=${row.paymentNo}`);
                  } else {
