...
export default function CardTable({ dataTestId }: { dataTestId: string }): React.ReactElement {
  ...
  const { filter, tableList, loading } = useSelector(cancelDoctorSalarySelector);
  const [isOpenConfirmDeleteModal, setIsOpenConfirmDeleteModal] = useState(false);

  useEffect(() => {
    if (loading) {
      void dispatch(getDoctorSalaryService({ data: filter }));
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string, record: unknown): React.ReactElement => {
        const row = record as DoctorSalaryDataType;
          ...
          <>
            <div className='flex justify-center gap-2'>
              <Page
                ...
                onClick={() => {
                  if (row.payType === 'X') {
                    router.push(`${URL.cancelPayment.cancelPaymentOfficeDetail.url}?id=${row.paymentNo}`);
                  } else if (row.payType === 'T') {
                    router.push(`${URL.cancelPayment.cancelPaymentBankeDetail.url}?id=${row.paymentNo}`);
                  } else if (row.payType === 'S') {
                    router.push(`${URL.cancelPayment.cancelPaymentChequeDetail.url}?id=${row.paymentNo}`);
                  } else if (row.payType === 'P') {
                    router.push(`${URL.cancelPayment.cancelPaymentMoneyDetail.url}?id=${row.paymentNo}`);
                  } else if (row.payType === 'M') {
                    router.push(`${URL.cancelPayment.cancelPaymentPromptPayDetail.url}?id=${row.paymentNo}`);
                  } else {
